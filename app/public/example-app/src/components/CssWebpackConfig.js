...
class CssWebpackConfig extends Component {
            ...
            oneOf: [
                {
                    ...
                    use: this.createLoaderList(rule, {
                        ...
                        auto: undefined,
                        localIdentName:
                            this.context.config.cssModuleIdentifier ||
                            "[hash:base64]",
                    }),
                },
                {
    ...
    excludePathsFor(command) {
        ...
        return exclusions.details.map((preprocessor) =>
            preprocessor.src.path()
        );
    }
    ...
    static wantsVueStyleLoader(context) {
        ...
        return (
            VueFeature &&
            VueFeature.options &&
            VueFeature.options.useVueStyleLoader
        );
    }
    ...
    static beforeLoaders({ context, type, injectGlobalStyles }) {
        ...
        if (context.globalStyles && injectGlobalStyles) {
            let resources =
                CssWebpackConfig.normalizeGlobalStyles(
                    context,
